...
from .serializers import MedicalBillRecordSerializer, MedicalBillPayloadSerializer, MedicalBillDocumentSerializer


class MedicalBillRecordViewSet(SaveUserMixin, viewsets.ModelViewSet):
    permission_classes = [permissions.IsAuthenticated]
    serializer_class = MedicalBillRecordSerializer
    parser_classes = [MultiPartParser, FormParser, JSONParser]

    def get_queryset(self):
        queryset = MedicalBillRecord.objects.filter(
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer = MedicalBillPayloadSerializer(data=payload)
        serializer.is_valid(raise_exception=True)
        validated = serializer.validated_data

        record = MedicalBillRecord(
            user=request.user,
        ...
        if request.FILES.getlist("documents"):
            for file in request.FILES.getlist("documents"):
                MedicalBillDocument.objects.create(
                    record=record,
                    file=file,
                    created_by=request.user,
                    updated_by=request.user
        ...
        return Response(
            {
                "message": "Medical bill record created successfully",
                "data": MedicalBillRecordSerializer(record).data
            },
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response(
            {
                "message": "Medical bill record updated successfully",
                "data": MedicalBillRecordSerializer(record).data
            },
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        record.save()
        return Response(
            {"message": "Medical bill record deleted successfully"},
            status=status.HTTP_200_OK
        )
    ...
    def add_document(self, request, pk=None):
        ...
        created_documents = []
        for file in files:
            document = MedicalBillDocument.objects.create(
                record=record,
                file=file,
                created_by=request.user,
                updated_by=request.user,
        ...
        return Response(
            {
                ...
                "data": MedicalBillDocumentSerializer(
    ...
    def _save_record_fields(self, record, validated):
        fields = [
            ...
            "record_hospital_name", "bill_type", "notes", "for_whom"
        ]
    ...
    def choices(self, request):
        return Response(dict(MedicalBillRecord.MEDICAL_BILL_TYPE_CHOICES))
